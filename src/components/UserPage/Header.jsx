...
export const Header = () => {
  ...
  return (
            ...
            <Grid size={{lg:12}} sx={{height:"111px",display: 'flex',alignItems:"center",justifyContent:"space-between"}}>
                        ...
                        <Box
                          sx={{
                            height: '170px',
                            width: '46%',
                            backgroundImage:"url(/assests/Images/header.png)",
                        ...
                        >
                       <img src="/assets/Images/logo2.png" alt="not found" style={{height:"60px",width:"60px",objectFit:"contain",marginLeft:"20px"}}/>
                       <Grid >
                       {items.map((item)=>(
                             <Link component="button"  onClick={()=>handlenav(item.id)} underline='none' sx={{
    color: "black",
    fontSize: "20px",
    mx: 4,
    ...
    '&::after': {
      ...
      height: '6px',
      width: '130%',
      background: item.id === 'home' && activePage ==='home'
        ? 'linear-gradient(to right,rgb(11, 84, 161),rgb(6, 72, 141))'
        : 'transparent',
      borderRadius: '6px',
      transition: 'background 0.3s ease',
    },
    '&:hover::after': {
      background: 'linear-gradient(to right, rgb(11, 84, 161),rgb(6, 72, 141))',
    }
  }}>{item.name}</Link>
                       ))}
